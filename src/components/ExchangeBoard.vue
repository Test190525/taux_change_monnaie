<template>
  <div class="exchange-board">
    ...
    <header class="board-header">
      ...
      <span class="api-badge">Options API</span>
    </header>
...
export default {
  ...
  computed: {
    convertedRates() {
      return this.currencies.map(currency => ({
        ...currency,
        amount: this.rates[currency.code] != null
          ? (this.xpfAmount * this.rates[currency.code]).toFixed(2)
          : '—'
      }))
    }
  },
  ...
  mounted() {
    this.fetchRates()
    this.intervalId = setInterval(this.fetchRates, 3600000)
  },
  ...
  methods: {
    async fetchRates() {
      ...
      try {
        ...
        this.error = `Impossible de charger les taux (${err.message}). Vérifiez votre clé API.`
